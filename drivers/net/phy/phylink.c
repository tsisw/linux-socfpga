				  ...
				  const struct phylink_link_state *state)
{
	...
	if (pl->qsfp_bus) {
		rate_kbd = phylink_interface_signal_rate(state->interface);
		if (rate_kbd)
			qsfp_upstream_set_signal_rate(pl->qsfp_bus, rate_kbd);
	}
				...
				struct phylink_link_state *state)
{
	...
	if (!phy_interface_empty(phy->possible_interfaces)) {
		...
		if (phy_on_qsfp(phy)) {
			/* If the PHY is on a QSFP, limit the interfaces to
			 * those that can be used with a QSFP module.
			 */
			phy_interface_and(interfaces, interfaces,
					  phylink_qsfp_interfaces);

			if (phy_interface_empty(interfaces)) {
				phylink_err(pl, "QSFP PHY's possible interfaces becomes empty\n");
				return -EINVAL;
			}
		}
