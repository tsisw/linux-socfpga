...
static struct qsfp_bus *qsfp_bus_get(const struct fwnode_handle *fwnode)
{
...
/**
 * qsfp_upstream_set_signal_rate() - set data signalling rate
 * @bus: a pointer to the &struct qsfp_bus structure for the qsfp module
 * @rate_kbd: signalling rate in units of 1000 baud
 *
 * Configure the rate select settings on the QSFP module for the signalling
 ...
 *  QSFP's sm_mutex
 */
void qsfp_upstream_set_signal_rate(struct qsfp_bus *bus, unsigned int rate_kbd)
{
	if (bus->registered)
		bus->socket_ops->set_signal_rate(bus->qsfp, rate_kbd);
}
EXPORT_SYMBOL_GPL(qsfp_upstream_set_signal_rate);
...
/**
 * qsfp_get_name() - Get the QSFP device name
 * @bus: a pointer to the &struct qsfp_bus structure for the qsfp module
 *
 * Gets the QSFP device's name, if @bus has a registered socket. Callers must
 * hold RTNL, and the returned name is only valid until RTNL is released.
 *
 * Returns:
 *	- The name of the QSFP device registered with qsfp_register_socket()
 *	- %NULL if no device was registered on @bus
 */
const char *qsfp_get_name(struct qsfp_bus *bus)
{
	ASSERT_RTNL();

	if (bus->qsfp_dev)
		return dev_name(bus->qsfp_dev);

	return NULL;
}
EXPORT_SYMBOL_GPL(qsfp_get_name);
